...
> **Historical record.** This plan was written against the Python/PyTorch
> proof-of-concept that lived at `src/resolve/` and was removed in
> [#106](https://github.com/gcol33/resolve/issues/106). Its `src/resolve/...`
> paths, its `Trainer(dataset, ...)` snippets, and its `compile_model` /
> `trainer.profile()` API all refer to that implementation and have no
> counterpart in the C++ engine. The measured hash-embedding numbers below were
> taken on the CUDA kernel in `src/core/cuda/kernels.cu`, which is still the
> engine's kernel; they are left exactly as measured. Kept for the reasoning
> and the measurements, not as instructions. For current guidance see
> `docs/tutorials/performance-tuning.md`.
