"""SpaccDataset: validated container for plot data."""
...
@dataclass
class SpaccSchema:
    """Schema information derived from a SpaccDataset."""
...
class SpaccDataset:
    ...
    def _validate(self) -> None:
        ...
        required_header = {self._roles.plot_id, self._roles.coords_lat, self._roles.coords_lon}
    ...
    def schema(self) -> SpaccSchema:
        ...
        return SpaccSchema(
            ...
            n_continuous=2 + len(self._roles.covariates),  # lat, lon + covariates
    ...
    def get_coordinates(self) -> np.ndarray:
        """Get (lat, lon) array for all plots."""
    ...
    def split(
        ...
        test_size: float = 0.2,
        seed: int = 42,
    ) -> tuple[SpaccDataset, SpaccDataset]:
        """
        ...
        train_ds = SpaccDataset(train_header, train_species, self._roles, self._targets)
        test_ds = SpaccDataset(test_header, test_species, self._roles, self._targets)

        return train_ds, test_ds

    def filter_by_target(self, name: str) -> SpaccDataset:
        ...
        return SpaccDataset(filtered_header, filtered_species, self._roles, self._targets)
