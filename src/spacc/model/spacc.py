"""SpaccModel: full model composing encoder and task heads."""
...
from spacc.data.dataset import SpaccSchema
from spacc.data.roles import TargetConfig
from spacc.model.encoder import PlotEncoder
from spacc.model.head import TaskHead


class SpaccModel(nn.Module):
    """
    Full Spacc model: shared encoder with multiple task heads.
    ...
    def __init__(
        ...
        # 2 (lat, lon) + len(covariates) + hash_dim
        n_continuous = 2 + len(schema.covariate_names) + hash_dim
