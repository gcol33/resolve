...
class RoleMapping:
    ...
    Required roles:
        plot_id: Unique identifier for each observation
        species_id: Species/item identifier in species table
        species_plot_id: Foreign key linking species rows to observations

    Optional roles:
        coords_lat: Latitude coordinate (optional)
        coords_lon: Longitude coordinate (optional)
    ...
    def validate(self) -> None:
        ...
        required = ["plot_id", "species_id", "species_plot_id"]
        ...
        if (self.coords_lat is None) != (self.coords_lon is None):
            raise ValueError("coords_lat and coords_lon must both be specified or both omitted")

    @property
    def has_coordinates(self) -> bool:
        return self.coords_lat is not None and self.coords_lon is not None
    ...
    def from_dict(cls, mapping: dict[str, str | list[str]]) -> "RoleMapping":
        ...
        return cls(
            ...
            coords_lat=mapping.get("coords_lat"),
            coords_lon=mapping.get("coords_lon"),
