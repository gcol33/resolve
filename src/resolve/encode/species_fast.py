...
class FastSpeciesEncoder:
    ...
    def transform(self, dataset: ResolveDataset) -> EncodedSpecies:
        ...
        # Use fast path if dataset has pre-loaded tensors from C++ loader
        if dataset.has_fast_species_tensors:
            return self._transform_from_tensors(dataset)
    ...
    def _transform_from_tensors(self, dataset: ResolveDataset) -> EncodedSpecies:
        """
        Fast path: use pre-loaded tensors from C++ loader.

        The C++ loader provides:
        ...
        - plot_offsets: (n_plots+1,) int64 - CSR offsets

        This avoids re-parsing the DataFrame and re-hashing species IDs.
        """
        tensors = dataset.fast_species_tensors
        plot_ids = dataset.plot_ids
        n_plots = len(plot_ids)

        # Extract tensors as numpy arrays
        plot_indices = tensors["plot_indices"].numpy().astype(np.int64)
        species_hashes = tensors["species_ids"].numpy().astype(np.int64)
        abundances = tensors["weights"].numpy().astype(np.float32)
        ...
        if self.normalization == "raw":
            ...
            # Compute plot totals using CSR offsets for efficiency
            plot_offsets = tensors["plot_offsets"].numpy()
            plot_totals = np.zeros(n_plots, dtype=np.float32)
            np.add.at(plot_totals, plot_indices, abundances)
        ...
        # Note: Taxonomy is not available in fast-loaded mode since the C++ loader
        # only loads plot_id, species_id, and weight columns. If taxonomy is needed,
        # fall back to the regular transform method.
        genus_ids = None
        family_ids = None

        # Unknown species tracking is not available in fast mode since we don't
        # have the original species ID strings to compare against vocabulary
        unknown_fraction = np.zeros(n_plots, dtype=np.float32)
        unknown_count = np.zeros(n_plots, dtype=np.int32) if self.track_unknown_count else None
