...
def pad_rank_pool_encoded(encoded: RankPoolEncodedSpecies) -> dict[str, np.ndarray]:
    ...
    Returns dict with:
        ...
        has_cover: (n_plots,) float32
    """
    n_plots = len(encoded.species_ids)
    if n_plots == 0:
        ...
        return {
            ...
            "has_cover": np.zeros(0, dtype=np.float32),
        }

    max_sp = max(len(s) for s in encoded.species_ids)
    ...
    for i in range(n_plots):
        n = len(encoded.species_ids[i])
        if n > 0:
            sp_ids[i, :n] = encoded.species_ids[i]
            g_ids[i, :n] = encoded.genus_ids[i]
            f_ids[i, :n] = encoded.family_ids[i]
            w[i, :n] = encoded.weights[i]
            mask[i, :n] = True
    ...
    return {
        ...
        "weights": w,
        "mask": mask,
        "has_cover": encoded.has_cover,
    }


class RankPoolEncoder(TaxonomyEncoderMixin):
    ...
    def transform(self, dataset: ResolveDataset) -> RankPoolEncodedSpecies:
        ...
        """
        if not self._fitted:
            raise RuntimeError("RankPoolEncoder must be fit before transform")
        ...
        if self.weighting == "binary":
            ...
            # Reciprocal rank weighting: sort by abundance descending within each plot,
            # assign dense ranks (ties get same rank), weight = 1/rank.
            df = df.with_columns(
        ...
        # has_cover: 1.0 for all plots when abundance data is available,
        # 0.0 when using binary weighting (no cover info).
        # Cover dropout (randomized at model level) will override this during training.
        has_cover = np.ones(n_plots, dtype=np.float32) if roles.has_abundance else np.zeros(n_plots, dtype=np.float32)
